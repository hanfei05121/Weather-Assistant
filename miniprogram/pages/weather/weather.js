...
const three_particles_1 = require("../../utils/three-particles");
...
Page({
    ...
    async init() {
        if (!(0, storage_1.getAutoLocated)() || (0, storage_1.getCurrentCity)() === '我的位置') {
    ...
    async autoLocate() {
        try {
            ...
            const city = await (0, api_1.getCityByLocation)(loc.latitude, loc.longitude);
            if (city) {
                const locName = `我的位置（${city.name}·${city.district}）`;
                (0, api_1.registerCityId)(locName, city.id);
                (0, api_1.registerCityCoord)(locName, { lat: loc.latitude, lon: loc.longitude });
                (0, storage_1.replaceLegacyLocationCity)(locName);
                (0, storage_1.setCurrentCity)(locName);
        ...
        (0, storage_1.setAutoLocated)(true);
    },
    getLocation() {
    ...
    onReady() {
        ...
        query.select('#weather-canvas').fields({ node: true, size: true }).exec((res) => {
            const info = res && res[0];
            if (!info || !info.node)
                return;
            const winInfo = wx.getWindowInfo ? wx.getWindowInfo() : wx.getSystemInfoSync();
            this.particles = new three_particles_1.ThreeWeatherParticles();
    ...
    onShow() {
        if (this.particles)
            this.particles.start();
        const currentCity = (0, storage_1.getCurrentCity)();
    ...
    async loadWeatherData() {
        ...
        try {
            ...
            const [nowRes, dailyRes, hourlyRes, airRes, indicesRes, warningRes] = await Promise.all([
                (0, api_1.getWeatherNow)(city),
                (0, api_1.getWeather10d)(city),
                (0, api_1.getWeather24h)(city),
                (0, api_1.getWeatherAir)(city).catch(() => null),
                (0, api_1.getWeatherIndices)(city).catch(() => null),
                (0, api_1.getWeatherWarning)(city).catch(() => null)
            ]);
            // 处理实时天气数据
            ...
            // 处理24小时预报数据
            const hourlyData = hourlyRes.hourly.slice(0, 24).map((item) => ({
            ...
            const dailyData = dailyRes.daily.map((item, index) => {
            ...
            if (airRes && airRes.indexes) {
                const qaqi = airRes.indexes.find((i) => i.code === 'qaqi') || airRes.indexes.find((i) => i.code === 'us-epa') || airRes.indexes[0];
                const pm25 = (airRes.pollutants || []).find((p) => p.code === 'pm2p5');
                const pm10 = (airRes.pollutants || []).find((p) => p.code === 'pm10');
                airData = {
            ...
            const warnings = ((warningRes && warningRes.alerts) || []).map((item) => ({
                id: item.id,
            ...
            this.setData({
                ...
                if (this.particles)
                    this.particles.setWeather(now.icon, parseInt(now.windSpeed) || 0);
            });
        }
        catch (error) {
